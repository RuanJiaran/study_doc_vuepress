...
- history 模式：url后面没有 # ，如http://127.0.0.1:5500/home/page1
...
**例如：**url  `http://127.0.0.1:5500/home/#/page1 `  的 `window.location.hash` 是 `#/page1`
...
`Route` 组件主要做的是通过 `BrowserRouter`  或 `HashRouter` 传过来的当前值，通过 `props` 传进来的 `path` 与 `context` 传进来的 `pathname` 进行匹配，然后决定是否执行渲染组件
